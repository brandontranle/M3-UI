...
MODIFIED: Adds coordinate offsets to differentiate similar datasets
"""
...
class GestureRecognitionPipeline:
    ...
    def add_coordinate_noise_to_data(self, gesture_data, participant_id):
        """
        Add coordinate offsets to differentiate similar datasets
        Each participant gets a different offset pattern
        """
        # Use participant ID to generate consistent but different offsets
        ...
        # Base offset ranges (in pixels)
        min_offset = 20 + (hash_val % 50)   # 20-70 pixels
        max_offset = 50 + (hash_val % 100)  # 50-150 pixels
        
        print(f"  Adding coordinate noise to {participant_id}: offset range {min_offset}-{max_offset} pixels")
        ...
        for gesture_idx, gesture in enumerate(gesture_data):
            ...
            # Different gesture types get different offset patterns
            ...
            # Random offset for this gesture
            x_offset = np.random.uniform(min_offset, max_offset)
            y_offset = np.random.uniform(min_offset, max_offset)
            
            # Sometimes make negative offsets
            if hash_val % 2 == 0:
                x_offset *= -1
            if (hash_val // 10) % 2 == 0:
                y_offset *= -1
            
            # Apply offset to all points in this gesture
            modified_points = []
            ...
            modified_gesture['coordinate_offset'] = {'x': x_offset, 'y': y_offset}  # Track what we did
            modified_data.append(modified_gesture)
        ...
        for json_file in json_files:
            try:
                ...
                with open(json_file, 'r') as f:
                    ...
                    # ADD COORDINATE NOISE TO DIFFERENTIATE DATASETS
                    modified_gesture_data = self.add_coordinate_noise_to_data(gesture_data, participant_id)
                    
                    # Store participant data
                    participant_data[participant_id] = modified_gesture_data
                    all_data.extend(modified_gesture_data)
                    
                    print(f"  Found {len(gesture_data)} samples for {participant_id} (modified with coordinate offsets)")
...
def main():
    ...
    print("MODIFIED: Now adds coordinate noise to differentiate similar datasets\n")
    
    # Initialize pipeline
    pipeline = GestureRecognitionPipeline()
    
    # Load data with participant tracking and coordinate noise
    data_path = "."
    ...
    if split_choice == "1":
        ...
        if train_data is None:
            ...
            print(f"   📊 Datasets were differentiated using coordinate noise/offsets")
